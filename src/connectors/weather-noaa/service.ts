...
export class NoaaWeatherConnectorService {
  private readonly alertsProvider: NoaaWeatherConnectorServiceOptions["alertsProvider"];
  private readonly eventPublisher: NoaaWeatherConnectorServiceOptions["eventPublisher"];
  ...
  constructor(options: NoaaWeatherConnectorServiceOptions) {
  ...
  async runOnce(): Promise<WeatherConnectorPollSummary> {
    ...
    const result = await this.alertsProvider.fetchActiveAlerts(this.maxAlertsPerPoll);
    ...
    for (const alert of result.alerts) {
      ...
      if (this.lastVersionByAlertId.get(alert.alertId) === version) {
      ...
      try {
        ...
        this.lastVersionByAlertId.set(alert.alertId, version);
        summary.published += 1;
      } catch (error) {
        summary.failed += 1;
